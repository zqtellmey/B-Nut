...
# ================= 配置区域 =================
# 修改点：优先获取 sing-box 生成的 PROXY_SOCKS5 变量，并确保使用 socks5h 以支持远程 DNS
PROXY_ENV = os.getenv("PROXY_SOCKS5") or os.getenv("PROXY")
if PROXY_ENV and PROXY_ENV.startswith("socks5://"):
    PROXY = PROXY_ENV.replace("socks5://", "socks5h://")
else:
    PROXY = PROXY_ENV
...
class BytenutRenewal:
    ...
    def __init__(self):
        ...
        # 修改点：初始化一个带代理的 session 用于后续 requests 调用
        self.session = requests.Session()
        if PROXY:
            print(f"[{time.strftime('%H:%M:%S')}] [INFO] Requests 使用代理: {PROXY}")
        ...
        try:
            if screenshot and os.path.exists(screenshot):
                ...
                with open(screenshot, "rb") as f:
                    # 修改点：此处使用带代理的 session
                    self.session.post(url, data={"chat_id": TG_CHAT_ID, "caption": msg}, files={"photo": f})
            else:
                url = f"https://api.telegram.org/bot{TG_TOKEN}/sendMessage"
                # 修改点：此处使用带代理的 session
                self.session.post(url, data={"chat_id": TG_CHAT_ID, "text": msg})
    ...
    def call_api(self, sb, url, referer=URL_HOMEPAGE, timeout=15):
        ...
        try:
            # 修改点：此处使用带代理的 session
            resp = self.session.get(url, headers=headers, cookies=cookies, timeout=timeout)
    ...
    def call_api_post(self, sb, url, referer=URL_HOMEPAGE, timeout=15):
        ...
        try:
            # 修改点：此处使用带代理的 session
            resp = self.session.post(url, headers=headers, cookies=cookies, timeout=timeout)
            if resp.status_code == 200:
                data = resp.json()
                if data.get('code') == 200:
                    return data.get('data')
